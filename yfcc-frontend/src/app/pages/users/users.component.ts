...
export class UsersComponent implements OnInit {
  ...
  ngOnInit() {
    this.commonservice.get('user/get').subscribe(data => {
      this.rows = this.temp = data.data;
    });
  }
  deleteRow(index, id) {
    if (window.confirm(`Are you sure to delete the record?`)) {
      this.commonservice.post('user/remove', { id }).subscribe(res => {
  ...
  filterTable($event) {
    ...
    this.temp = this.rows.filter((d) => {
      ...
      if (d.email.toLowerCase().indexOf(val) !== -1) {
        return true;
      }
      if (d.role.toLowerCase().indexOf(val) !== -1) {
        return true;
      }
